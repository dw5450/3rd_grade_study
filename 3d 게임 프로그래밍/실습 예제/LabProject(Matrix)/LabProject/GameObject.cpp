...
CVector3 CVector3::Normalize(CVector3& vector)										//벡터의 정규화
{
...
float CVector3::DotProduct(CVector3& vector1, CVector3& vector2)					//내적
{
	return(vector1.x * vector2.x + vector1.y * vector2.y + vector1.z * vector2.z);
}

CVector3 CVector3::CrossProduct(CVector3& vector1, CVector3& vector2)				//외적
{
...
CMatrix CMatrix::Multiply(CMatrix& mtxMatrix1, CMatrix& mtxMatrix2)					//행렬의 곱
{
	CMatrix mtxResult;
...
CMatrix CMatrix::RotationYawPitchRoll(float fYaw, float fPitch, float fRoll)						//x축, y축, z축 회전
{
	CMatrix mtxResult;
	if (fPitch)
	{
...
CMatrix CMatrix::RotationAxis(CVector3& vAxis, float fAngle)									//축을 회전 시킨다...?											//좌표계를 바꾼다?
{
	CMatrix mtxResult;
...
CMatrix CMatrix::PerspectiveFovLH(float fovy, float r, float zn, float zf)						//투영변환
{
	CMatrix mtxResult;
	mtxResult._22 = 1.0f / float(tan(DegreeToRadian(fovy * 0.5f)));
	mtxResult._11 = mtxResult._22 / r;															//화면 비율 보정
	mtxResult._33 = zf / (zf - zn);																//근평면
...
CMatrix CMatrix::LookAtLH(CVector3& vCamera, CVector3& vLookAt, CVector3& vUp)					//카메라를 위한 월드 변환 행렬과 카메라 변환 행렬
{
	...
	CMatrix mtxResult;
	mtxResult._11 = vRight.x;	mtxResult._12 = vUp.x;	mtxResult._13 = vLook.x;
	mtxResult._21 = vRight.y;	mtxResult._22 = vUp.y;	mtxResult._23 = vLook.y;	
	mtxResult._31 = vRight.z;	mtxResult._32 = vUp.z;	mtxResult._33 = vLook.z;
...
CVector4 CMatrix::Transform(CVector3& vPosition, CMatrix& mtxTransform)		//아핀 변환										//이동
{
	CVector4 vResult;
...
//점에 변환 행렬 적용
CVector3 CMatrix::TransformCoord(CVector3& vPosition, CMatrix& mtxTransform)			//3X3 변환							//회전			
{
	CVector3 vResult;
	vResult.x = vPosition.x * mtxTransform._11 + vPosition.y * mtxTransform._21 + vPosition.z * mtxTransform._31 + mtxTransform._41;
	vResult.y = vPosition.x * mtxTransform._12 + vPosition.y * mtxTransform._22 + vPosition.z * mtxTransform._32 + mtxTransform._42;
...
//노말 벡터에 변환 행렬 적용후 다시 노말 백터로 변경해서 반환
...
CPolygon::CPolygon(int nVertices)												//객체 생성자
{ 
	if (nVertices > 0)
	{
...
CPolygon::~CPolygon()															//객체 소멸자 
{ 
	if (m_pVertices) delete [] m_pVertices; 
}

void CPolygon::SetVertex(int nIndex, CVertex vertex)							//해당 인덱스의 점값 변경
{
...
void CPolygon::Draw(HDC hDCFrameBuffer, CMatrix& mtxWorldViewProjection, CCamera *pCamera)
{
    ...
    {
		CVertex vModel = m_pVertices[i % m_nVertices];				//마지막 점을 처음 점으로 받기 위해서
//World/View/Projection Transformation(Perspective Divide)
		...
		if (vCurrent.z != 0.0f) { vCurrent.x = vCurrent.x / vCurrent.w; vCurrent.y = vCurrent.y / vCurrent.w; vCurrent.z = vCurrent.z / vCurrent.w; }	//원근감 표현

//		if ((vCurrent.z < 0.0f) || (vCurrent.z > 1.0f)) continue;
//Screen Transformation				//스크린 변환
		//보정치가 없다?>
...
CMesh::CMesh(int nPolygons)
{
	if (nPolygons > 0)
	{
		...
		m_nReferences = 0;									//래퍼런스가 무엇을 의미하는거지
	}
}
...
void CMesh::SetPolygon(int nIndex, CPolygon *pPolygon)					//면을 설정해주는 함수
{
...
CCamera::CCamera()
{
	...
	CMatrix::Identity(m_mtxView);								//카메라 매트릭스의 역
	CMatrix::Identity(m_mtxProjection);							//단위 행렬로 저장
	CMatrix::Identity(m_mtxViewProject);						//단위 행렬로 저장
...
void CCamera::GenerateViewMatrix()												//카메라 좌표계가 원점으로 하는 행렬로 변환해주는 함수
{
...
void CCamera::SetLookAt(CVector3& vPosition, CVector3& vLookAt, CVector3& vUp)				//점의 포지션, 바라보는 곳이 포지션, 
{
