...
spam_CalPower(PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "fffffffffffffff",&힘or지능		, &공격력		, &크리티컬		,&속성강화
											,&힘지능증가		, &물리마법독립공격력증가	, &증가데미지
											,&추가증가데미지	, &크리티컬증가데미지		, &추가크리티컬증가데미지 
											,&추가데미지		, &속성추가데미지
											,&모든공격력증가	, &스킬공격력증가,		&방어력감소)); // 매개변수 값을 분석하고 지역변수에 할당 시킵니다.
	...
	전투력 = (1 + (힘or지능)* (1 + 힘지능증가 / 100))/250 * (공격력 * (1 + 물리마법독립공격력증가 / 100)) 
		...
		* (1 + 추가데미지 / 100 + 속성추가데미지/100 * (1.05 + 0.0045 * 속성강화))
		* (1 + 모든공격력증가 / 100) * (1 + 스킬공격력증가 / 100) * (1 + 방어력감소 * 1.163);
