...
public:
	...
	void playPlayers()
	{
		...
		//나부터 실행
		temp_score = static_cast<int>(escapeDistribution(dre));										//랜덤적으로 값을 추출
		...
		if (player->getEscapeScore() < temp_score)										//플레이어의 훈련소 탈출 점수가 랜덤 추출 값보다 작을 경우
			player->setEscapeScore(temp_score);											//랜덤 추출 값으로 값을 갱신
																						...
																						//나머지 플레이어 실행
		played_indexs.reserve(player_num);
		played_cnt = 1;						//실행된 플레이어 수 저장				//'나'를 실행 했으므로 1부터 
		player_index = 0;					//플레이할 플레이어 인덱스
	...
	//설정한 플레이 수 만큼의 플레이어 실행 (중복 x)
		...
		if (1 < escape_rank && escape_rank < player_num) {
			shared_ptr<Player> prevPlayer = players_sorted_school[escape_rank - 2];
			shared_ptr<Player> nextPlayer = players_sorted_school[escape_rank];
